#!/bin/bash
input_folder='/mnt/DATA1/GEO-1/04-hr'
output_folder='/mnt/DATA1/GEO-1/04-hr2'
...
# 遍历文件夹中的所有fastq文件
for file in "$input_folder"/*_fastp_1.fastq.gz; do
    # 获取文件名和路径
    filename=$(basename "$file")
    # 提取文件名前第一个_前的部分
    file_prefix="${filename%%_fastp_1.fastq.gz}"
    ...
    # 构建第二端fastq文件的路径
    input_R2="$input_folder/${file_prefix}_fastp_2.fastq.gz"
